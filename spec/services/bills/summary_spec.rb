...
RSpec.describe Bills::Summary do
  ...
  let(:bill) { create(:bill, status: :active, title: "Observatory Small Plates") }
  ...
  describe "partially assigned bill" do
    ...
    let!(:tax) do
      create(:receipt_adjustment, receipt: receipt, label: "VAT (included)", kind: :tax, amount_cents: 3_450, affects_total: false, position: 1)
    end
    let!(:delivery) do
      create(:receipt_adjustment, receipt: receipt, label: "Delivery", kind: :delivery_fee, amount_cents: 500, affects_total: true, position: 2)
    end
    ...
    it "returns bill summary fields" do
      expect(summary[:bill]).to include(
        id: bill.id,
    ...
    it "returns receipt adjustments with affects_total" do
      expect(summary[:receipt_adjustments]).to contain_exactly(
        hash_including(id: tax.id, kind: "tax", affects_total: false),
        hash_including(id: delivery.id, kind: "delivery_fee", affects_total: true)
      )
    end
  end

  describe "fully assigned bill with settled participants" do
    let!(:katlego) { create(:bill_participant, bill: bill, name: "Katlego", settled: true, seat_index: 0) }
    let!(:alex) { create(:bill_participant, bill: bill, name: "Alex", settled: true, seat_index: 1) }
    let!(:burger) { create(:receipt_item, bill: bill, receipt: receipt, name: "Burger", unit_price_cents: 8_000, total_cents: 8_000, position: 0) }
    ...
    it "uses receipt stored total when present" do
      receipt.update!(total_cents: 8_000)
